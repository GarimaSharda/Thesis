...
df <- read_csv("C:/Users/garim/Downloads/la_liga_stats_2010_2015_detail.csv",
...
eda <- df %>%
  transmute(
    ...
    # career length (includes actives + dead)
    career_length = career_lenght_w_actives_and_dead,
    
    # retirement status and age
    retired             = retired_tra,          # "Yes"/"No"
    retirement_age      = death_age,           # NA if not deceased
    retirement_date_clean = retirement_date_tra,
    
    # ages
    age          = age,
    age_current  = `current age_fbref`,
    
    # debut age and timing
    debut_age_transfermarket = debut_age_transfermarket,
    debut_age_num = suppressWarnings(as.numeric(debut_age_transfermarket)),
    debut_year    = debut_year_fbref,
    debut_season  = debut_season_fbref,
    first_tier_debut_age_raw = first_tier_debut_age,
    ...
    injury_seasons     = injury_seasons_tra,
    
    # matches
    total_matches_club  = `CAREER Sum of matches_played`,
    total_matches_total = `CAREER Sum of matches_played`,   # no separate total given
    
    # position / nation / height / national team
    position_simple = position_tra,
    position_clean  = position_tra,
    nation          = citizenship_tra,
    height.y        = height_tra,
    nat_team_flag   = ifelse(National_team == "Yes", "Yes", "No")
  ) %>%
  mutate(
    # debut‑age bands
    debut_band = cut(
